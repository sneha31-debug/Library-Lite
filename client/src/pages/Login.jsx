...
const Login = () => {
    ...
    const googleLogin = useGoogleLogin({
        onSuccess: async (tokenResponse) => {
            ...
            try {
                ...
                console.log('Backend Google Auth Response:', response.data); // Debug log
    ...
    // Load error from sessionStorage on mount
    useEffect(() => {
        const savedError = sessionStorage.getItem('loginError');
        if (savedError) {
            setErrors({ submit: savedError });
            console.log('Loaded error from sessionStorage:', savedError);
        }
    }, []);

    // Prevent component from clearing errors on re-render
    useEffect(() => {
        console.log('Login component mounted/updated, errors:', errors);
    }, [errors]);
    ...
    const handleSubmit = async (e) => {
        ...
        if (validateForm()) {
            ...
            if (result.success) {
                ...
                sessionStorage.removeItem('loginError'); // Clear error on success
                navigate('/');
            } else {
                console.log('Login failed:', result.error); // Debug log
                sessionStorage.setItem('loginError', result.error); // Persist error
    ...
    const handleSignupClick = () => {
        sessionStorage.removeItem('loginError'); // Clear error when going to signup
        navigate('/signup');
    };
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <div className="grid grid-cols-1 gap-4">
                            ...
                            >
                                ...
                                <span className="text-sm font-semibold text-[#1a1a1a]">Sign in with Google</span>
                            </button>
                        </div>
