...
const Login = ({ onNavigate }) => {
    ...
    const handleSocialLogin = async (provider) => {
        ...
        try {
            ...
            await socialLogin(provider, mockData.email, mockData.name, mockData.photoUrl);
            if (onNavigate) onNavigate('home');
        } catch {
            setErrors({ form: `Failed to login with ${provider}` });
        } finally {
                ...
                <div className="bg-white rounded-3xl shadow-2xl p-8 md:p-12">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <button
                            type="submit"
                            disabled={isLoading}
                            className="w-full bg-[#3d4f3d] text-[#e8e89a] py-3 rounded-xl font-semibold hover:bg-[#2a3b2a] transition-colors shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 transition-all disabled:opacity-70 disabled:cursor-not-allowed"
                        >
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <button
                                ...
                                className="flex items-center justify-center gap-2 px-4 py-3 border-2 border-gray-200 rounded-xl hover:bg-gray-50 transition-colors disabled:opacity-70 disabled:cursor-not-allowed"
                            >
                                {socialLoading === 'Google' ? (
                                    <div className="w-5 h-5 border-2 border-[#3d4f3d] border-t-transparent rounded-full animate-spin"></div>
                                ) : (
